...
export function getCategoryFromResponse(
...
export async function analyzeExperiment(
  ...
  await braintrust.withExperiment(
    ...
    async (experiment: braintrust.Experiment) => {
...
export async function runOnAllTitles(
  ...
  const promises = titles.map((title) =>
    classifyTitle(openai, prompt, title.text)
  );
  const ret = await Promise.all(promises);
  const endTime = performance.now();
  console.log(
    `Processed ${titles.length} titles in ${
...
export async function loadDataset(): Promise<Dataset> {
  ...
  const titles: Title[] = [];
  for (const row of jsonData["rows"].slice(0, 10)) {
    titles.push({
  ...
  const categoriesFeature = jsonData["features"].find(
    (f: Record<string, unknown>) => {
      return f["name"] == "label";
    }
  );

  const categories = (categoriesFeature?.type?.names || []).map((c: string) =>
    c.toLowerCase()
  );
